...
CancelablePromise is a Promise that can be canceled. It is a Promise that has a status property that can be '`pending`', '`resolved`', '`rejected`' or '`canceled`'. It has an onCancel method that allows to register a callback that will be called when the promise is canceled. It has a cancel method that allows to cancel the promise.
...
```ts
import { createDecoupledPromise } from 'async-callbacks-manager';
...
```ts
import { toCancelablePromise } from 'async-callbacks-manager';
...
```ts
import { groupAsCancelablePromise } from 'async-callbacks-manager';
...
```ts
import { isPromise } from 'async-callbacks-manager';
...
```ts
import { tryCatch } from 'async-callbacks-manager';
...
```ts
import { tryCatchPromise } from 'async-callbacks-manager';
...
We welcome contributions to async-callbacks-manager! If you have an idea for a new feature or improvement, please open an issue or submit a pull request.

# License

Async-callbacks-manager is released under the MIT License.
